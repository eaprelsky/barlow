...
export const GUIDES: Guide[] = [
  {
    id: 'sound-design', title: 'собрать развивающийся тембр', goal: 'Слои, MSEG и новые способы синтеза', section: 'more',
    steps: [
      { target: '[data-ob="mode-inst"]', say: 'Открой инструмент выбранной дорожки.', expect: 'click' },
      { target: '[data-ob="instrument-layers"]', say: 'В «слоях» можно сложить основной голос и ещё три.', hint: 'Добавь пресет, открой имя слоя и нажми «редактировать источник…». Предварительное сохранение не нужно. Уровень 0 выключает голос; частота × меняет отношение высот.' },
      { target: '[data-ob="tab-snd"]', say: 'Открой источник основного голоса.', expect: 'click' },
      { target: '[data-ob="src-seg"]', say: 'Для синтеза выбери волну; для записи — сэмпл.', hint: 'Под волной доступны операторы/FM, кадры wavetable и аналоговые формы VA. У wavetable доступны проход туда-обратно и LFO позиции.' },
      { target: '[data-ob="tab-env"]', say: 'Открой огибающую — форму громкости ноты.', expect: 'click' },
      { target: '[data-ob="envelope-mode"]', say: 'Режим «по точкам» позволяет нарисовать несколько атак и пауз.', hint: 'Готовая форма помогает начать. Изгиб меняет ход перехода. Удержание ждёт конца ноты, затем проигрывает затухание; «туда-обратно» добавляет повторы. Обе крайние точки тихие.' },
      { target: '[data-ob="tab-timbre"]', say: 'Открой окраску и фильтры.', expect: 'click' },
      { target: '[data-ob="voice-color"]', say: 'Ring даёт металл, wavefold — гармоники, comb — резонанс.', hint: 'Начинай с одной обработки и небольшой доли. Нулевое значение выключает её.' },
      { target: '[data-ob="preview-in-track"]', say: 'Прослушай звук в регистре своей партии.', expect: 'click' },
      { target: '[data-ob="save-inst"]', say: 'Сохрани готовый инструмент в «мои».', hint: 'Весь состав голосов и их настройки сохранятся вместе. «В файл» рядом создаёт переносимый архив с записями; получатель добавит его через «из файла…» в библиотеке.' },
    ],
  },
  {
    id: 'main',
  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        say: 'Вкладка «огибающая»: громкость, высота и локальный фильтр. Начни с громкости; для райзера добавь подъём высоты или открытие фильтра.',
        side: 'bottom',
      },
      {
        target: '[data-ob="tab-timbre"]',
        say: 'Вкладка «тембр»: фильтры, ring, wavefold и comb. Унисон и вибрато — на «источнике».',
        side: 'bottom',
      },
      {
  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        say: 'Двойной клик по сцене — имя. Крестик рядом с плюсом удаляет выбранную сцену; «цепочка» открывает порядок частей.',
        side: 'bottom',
      },
      {
      ...
      },
      {
        ...
        open: 'chain',
        say: 'В плитке выбери сцену, такты и общий или свой BPM. Перетаскивай номер, чтобы поменять порядок.',
        side: 'bottom',
      },
    ],
  },
  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        hint: 'Подборка сужает список по задаче: ритм, бас, мелодия, атмосфера. Папка в шапке добавляет инструмент из файла.',
        side: 'bottom',
      },
      {
        target: '[data-ob="inst-cards"] .inst-card',
  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        say: 'Жми «сэмпл → в волну» — слепок сразу станет волной инструмента.',
        expect: 'click',
        side: 'bottom',
      },
      {
        target: '[data-ob="we-draft-row"]',
        say: 'Правки — в черновике: «▶ нота» в шапке слушает его, «применить» делает таблицей инструмента.',
        hint: 'На канвасе: яркая линия — черновик, приглушённая — звучащий тембр.',
        side: 'bottom',
      },
      {
        target: '[data-ob="we-partials"]',
        say: 'Строки-операторы: ×2 — октава, хвост — звон, «куда» — модуляция другой строки (FM).',
        hint: 'Справа — унисон, вибрато, форманты и заготовка: слои поверх любой таблицы.',
        side: 'top',
      },
    ],
  },
  ...
  },
  {
    ...
    steps: [
      ...
      {
        target: '[data-ob="file-menu"]',
        say: '«Экспортировать WAV…» — аудио. «Сохранить проект…» — ZIP со всеми записями.',
        hint: '«Открыть проект…» понимает JSON и ZIP. Импорт инструмента — отдельная команда.',
        side: 'bottom',
      },
    ],
  },
