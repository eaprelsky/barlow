...
export function MsegEditor({ value, onChange }: { value: Mseg; onChange: (value: Mseg) => void }) {
  ...
  return <div className="mseg-editor" data-ob="mseg">
    ...
    <div className="mseg-toolbar">
      ...
      <label>время, % <NumField help="mseg-time" ariaLabel="Время точки, %" value={point.t * 100} min={interior ? (value.points[i - 1].t + .001) * 100 : 0} max={interior ? (value.points[i + 1].t - .001) * 100 : 100} disabled={!interior} step={.1} onChange={t => edit(i, t / 100, point.v)} /></label>
      <label>уровень, % <NumField help="mseg-level" ariaLabel="Уровень точки, %" value={point.v * 100} min={0} max={100} disabled={!interior} step={1} onChange={v => edit(i, point.t, v / 100)} /></label>
