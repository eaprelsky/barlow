...
export function BridgeSettings({ session, status, onConnect, onDisconnect }: {
  ...
  return <details className="bridge-settings">
    ...
    <form onSubmit={e => {
      ...
      <label>Код подключения <input type="password" autoComplete="off" spellCheck={false} value={code} maxLength={80}
        onChange={e => setCode(e.target.value)} /></label>
      <label><input type="checkbox" checked={write} onChange={e => setWrite(e.target.checked)} />редактировать проект</label>
      <label><input type="checkbox" checked={transport} onChange={e => setTransport(e.target.checked)} />управлять воспроизведением</label>
