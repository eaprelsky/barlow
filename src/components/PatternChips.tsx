...
export function PatternChips({
  ...
  return (
    <div className="pattern-chips" data-ob="chips">
      {/* Мьют — «отрицательный эскиз» сцены: дорожка молчит, пока сцена
          держит этот эскиз, часы партии идут. Клик по любому чипу — как
          выбор «настоящего» эскиза — снимает тишину (мьют — свойство
          сцены, не эскиза). */}
      <button
        className={slotMuted ? 'chip mute on-m' : 'chip mute'}
        data-ob="chip-mute"
        title="Мьют в этой сцене: дорожка молчит, пока сцена держит этот эскиз. Часы идут — сняв мьют, войдёшь в фазе. Снимается повторным кликом или выбором другого эскиза. В других сценах этот эскиз играет как обычно"
      ...
      {track.patterns.map((pt) => {
        ...
        return (
          <button
            key={pt.id}
            className={'chip' + (pt.id === pattern.id ? ' on' : '')}
            title={
              (pt.forkedFrom
              ...
              (slotMuted ? '. Партия заглушена мьютом — клик включит её' : '') +
