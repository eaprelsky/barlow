...
export function NoteLocksEditor({ note, sounding, onChange }: {
  ...
  return <details className="note-locks" data-note-locks={note.n}>
    ...
    <div className="note-lock-list">
      {fields.map(field => {
        ...
        return <div className="note-lock" key={field}>
          ...
          {!lockApplicable(field, st) && <span>не действует в этом режиме источника</span>}
          <button aria-label={`Снять фиксацию: ${spec.label}`} onClick={() => remove(field)}>снять</button>
        </div>;
      })}
    </div>
    <select aria-label={`Добавить параметр ноты ${note.n + 1}`} value="" disabled={fields.length >= NOTE_LOCK_LIMIT || !available.length}
      ...
      <option value="">+ параметр ноты…</option>
