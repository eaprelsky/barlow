...
export function makeSceneSpace(ctx:BaseAudioContext,dest:AudioNode,sizeSec:number,level:number,seed?:number) {
 ...
 return {input,closeAt(at:number){gain.gain.cancelScheduledValues(Math.max(ctx.currentTime,at-.005));gain.gain.setValueAtTime(level,Math.max(ctx.currentTime,at-.005));gain.gain.linearRampToValueAtTime(0,at);},update(size:number,value:number){level=value;const ir=getImpulse(ctx,size,seed);if(ir!==reverb.buffer)reverb.buffer=ir;gain.gain.setTargetAtTime(value,ctx.currentTime,.03);},dispose(){input.disconnect();reverb.disconnect();gain.disconnect();}};
}
