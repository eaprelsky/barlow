...
function noteCost(st: SoundingTrack): number {
  ...
  return 20 + motion + (st.ringMix ? 6 : 0) + (st.foldDrive ? 2 : 0) + (st.combMix ? 7 : 0) + (st.ampMseg ? 1 : 0) + (st.pitchMseg ? 1 : 0) + (st.filterMseg ? 1 : 0) + (st.mono && (st.portamentoSec ?? 0) > 0 ? 1 : 0) + body + 2 * (st.formants?.length ?? 0);
}
