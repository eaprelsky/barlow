...
export function voiceLifetimeBound(st: SoundingTrack, notes: Note[], stepSec: number, durSec?: number): number {
  ...
  for (const nt of notes) {
    ...
    const base = durSec ?? (sound.noteSteps && sound.noteSteps > 0 ? sound.noteSteps * stepSec : attack + sound.decay);
    ...
    if (durSec === undefined && !sound.noteSteps && nt.len === undefined && (sound.sustain ?? 0) >= .99) len = Math.max(len, 16);
    ...
    longest = Math.max(longest, sound.waveform === 'sample' && sound.sampleMode === 'grain' ? grain : Math.max(body, partials) + .1);
  }
  return longest;
}
