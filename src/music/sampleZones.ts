...
export function sampleAssets(source: { layers?: { sound: Parameters<typeof sampleAssets>[0] }[]; sampleId?: string; sampleName?: string; sampleZones?: SampleZone[]; sampleSlices?: SampleSlice[] }): { sampleId: string; sampleName?: string }[] {
  ...
  for (const layer of source.layers ?? []) for (const asset of sampleAssets(layer.sound)) result.set(asset.sampleId, asset);
