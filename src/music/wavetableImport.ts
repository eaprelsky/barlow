...
export function readTableWav(buffer: ArrayBuffer): Float32Array {
  if (buffer.byteLength > 16 * 1024 * 1024) throw Error(msg("wavetableImport.theWAVExceeds16MiB"));
  const v = new DataView(buffer), tag = (n: number) => String.fromCharCode(...new Uint8Array(buffer, n, 4));
  if (buffer.byteLength < 44 || tag(0) !== 'RIFF' || tag(8) !== 'WAVE') throw Error(msg("wavetableImport.aPCMOrFloat32WAVIsRequired"));
  ...
  for (let p = 12; p + 8 <= v.byteLength;) {
    ...
    if (next > v.byteLength) throw Error(msg("wavetableImport.theWAVIsTruncated"));
  ...
  if (![1, 2].includes(channels) || !((format === 1 && [16, 24, 32].includes(bits)) || (format === 3 && bits === 32)) || block !== channels * bits / 8 || !bytes || bytes % block) throw Error(msg("wavetableImport.monoOrStereoPCM162432And"));
  ...
  for (let i = 0; i < samples.length; i++) for (let c = 0; c < channels; c++) {
    ...
    if (!Number.isFinite(x)) throw Error(msg("wavetableImport.theWAVContainsInvalidValues"));
...
export function importTable(samples: Float32Array, size: number, count: number): number[][] {
  if (![128, 256, 512, 1024, 2048, 4096].includes(size) || samples.length % size || samples.length < size || !Number.isInteger(count) || count < 2 || count > 8) throw Error(msg("wavetableImport.theWAVLengthMustBeAMultiple"));
  ...
  if (peak < 1e-8) throw Error(msg("wavetableImport.theTableContainsNoAudibleSignal"));
