...
const out = await page.evaluate(async (fixture) => {
  ...
  return [...d.slice(2205, 2205 + 2000)].map((v) => +v.toFixed(5));
}, fixture);
writeFileSync('fixtures/tmp-web-t1.json', JSON.stringify(out));
console.log('web samples saved:', out.length, 'head:', out.slice(0, 5));
await browser.close();
vite.kill();
