...
- Библиотека звуков — левая док-панель `SoundBrowser.tsx` (бывшая модалка
  ...
  «+ трек» сразу открывает панель с новым треком целью. Строй пресета
  (шкала/тоника/октавы) встаёт ТОЛЬКО пустому треку: у сыгранного пресет
  меняет тембр, не высоты. Новый трек наследует строй от последнего трека,
  у которого его правили (lastScaleRef в перехваченном setPatch), а не от
  верхнего в списке.
