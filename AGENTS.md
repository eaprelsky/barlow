...
- Модель арранжмента: паттерн (эскиз) → сцена (slots: trackId→patternId) →
  ...
  (rate) — на эскизе (v33; до этого — на треке с переопределением,
  normalize переносит старые патчи). Владение остальным — docs/DESIGN.md
  («Владение параметрами»): звук инструмента, эффекты и огибающая — трек;
  ноты, шаг, громкость/пан, вход/выход, модуляции — эскиз (блок эскиза
  в TrackRow).
