...
- Октавы стана: Note.n — индекс отсортированного массива строк scaleOf; добавление/удаление октавы пересобирает индексы — дельта считается по факту (шкала с отношением 2 схлопывается), правки — через onTrackCommand (отдельный шаг undo). Листалка окна ▲/▼ (между «+окт» и «−», состояние rollView в TrackRow) — UI, не патч: окно {lo, rows} лишь срезает строки рендера, ноты живут в абсолютных индексах мира (ушедшие за окно играют, но не видны); у края диапазона листалка доращивает октаву через onTrackCommand (снизу — с компенсацией нот, как +окт). Драг ноты без выделения: движение превращает её в выделение и переносит; цель драга/рамки считается из координат pointerup (последний pointermove могло коалесцировать).
...
- Модель арранжмента: паттерн (эскиз) → сцена (slots: trackId→patternId) →
  ...
  длина ноты ПО УМОЛЧАНИЮ (рисовалка новых, «нота» в тулбаре стана),
  фаза, шкала/тоника, эффекты, арп, сайдчейн — трек; ноты (в т.ч. своя
...
- В модалках с «?»-хинтом (шкала, выбор сэмпла) первая кнопка модалки —
